...
var accordion = function accordion(triggersSelector, itemsSelector) {
  var btns = document.querySelectorAll(triggersSelector),
      blocks = document.querySelectorAll(itemsSelector); // Вариант 1
  // function hideItems() {
  //     btns.forEach(btn => {
...
var drop = function drop() {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      arr[0].length > 16 ? dots = '...' : dots = '.';
      var name = arr[0].substring(0, 16) + dots + arr[1];
      input.parentElement.firstElementChild.textContent = name;
...
var filter = function filter() {
  ...
  function filter(btnClass, markClass) {
    var btn = menu.querySelector(btnClass),
...
var forms = function forms(state) {
  ...
  var clearInputs = function clearInputs() {
    ...
    upload.forEach(function (item) {
      item.parentElement.firstElementChild.textContent = 'Загрузить фотографию';
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots;
      var arr = item.files[0].name.split('.'); // 'qwertyasd.jpg' => ['qwertyasd', 'jpg']

      arr[0].length > 16 ? dots = '...' : dots = '.';
      var name = arr[0].substring(0, 16) + dots + arr[1];
      item.parentElement.firstElementChild.textContent = name;
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      console.log(api);
      var formData = new FormData(item); // if (item.classList.contains('calc_form')) {
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    function openModal(modalSelector, timer) {
      ...
      gift.style.right = "calc(2rem + ".concat(scroll, "px");
    }
    ...
    modal.addEventListener('click', function (e) {
      if (e.target && e.target === modal || e.target && e.target.classList.contains(closeSelector.slice(1))) {
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    ...
    img.src = img.src.slice(0, -4) + '-1.png'; //Отрезали .png и пришили -1.png
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); // Стилями
  // cards.forEach(item => {
  ...
  btn.addEventListener('click', function () {
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/styles').then(function (res) {
      ...
      return btn.textContent = 'Упс... Что-то пошло не так';
    });
  });
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      var src = _ref.src,
          title = _ref.title,
          link = _ref.link;
      //Сразу же деструктурировали item, поскольку это обьект
  ...
  function activateAnimation() {
    if (dir === 'vertical') {
      paused = setInterval(function () {
        //В переменную записываем уникальный id того интервала который запускаем
        plusSlides(1);
